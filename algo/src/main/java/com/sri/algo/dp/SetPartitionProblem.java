...
public class SetPartitionProblem {
	 ...
	 * 
	 * T.C : O(2^n) as each node has two children.
	 * 
	 * @param arr
	 * @return
	 */
	...
	public static boolean isSubSetSum(int[] arr, int n, int sum, String path) {
		...
		return isSubSetSum(arr, n - 1, sum, path) || isSubSetSum(arr, n - 1, sum - arr[n - 1], arr[n - 1] + "," + path);
	}
	...
	public static boolean canPartition(int[] nums) {
		...
		// sum should not be odd.. it has to be even
		if ((sum & 1) == 1) {
			return false;
		}
