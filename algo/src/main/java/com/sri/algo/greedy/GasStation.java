...
public class GasStation {
	 ...
	 * T.C : O(n*n)
	 * 
	 * Start with every station and see if we can reach it back..
	 * 
	 * @param gas
	 * @param cost
	 * @return
	 ...
	 * 
	 * 1) If car starts at A and can not reach B. Any station between A and B can not
	...
	public int canCompleteCircuit(int[] gas, int[] cost) {
		...
		for (int i = 0; i < n; i++) {
			...
			// We cannot reach i from previous start so reset start ..
			if (tank < 0) {
				total += tank;  // total gas collected till i..
				start = i + 1; // reset start to i + 1;
